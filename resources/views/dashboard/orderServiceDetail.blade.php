            ...
            <form method="POST" action="{{ route('dashboard.orderService.update', $order) }}" class="space-y-8 mt-[100px]">
                ...
                <div class="flex flex-col md:items-end items-center md:mr-[150px] mr-0 md:mt-6">
                    <div
                        ...
                        <p class="md:text-xl text-sm  text-[#000000]">Nª-
                            {{ $order->folio_number }}
                        </p>
                    </div>
                </div>
                    ...
                    <div class="form-client-data">
                        ...
                        <div class="form-group">
                            ...
                            <div
                                ...
                                <select id="client_document_type" name="client_document_type"
                                    ...
                                    <option value="cc" @selected(old('client_document_type', $order->vehicle->client->document_type ?? '') === 'cc')>C.C.</option>
                                    <option value="nit" @selected(old('client_document_type', $order->vehicle->client->document_type ?? '') === 'nit')>NIT</option>
                                    <option value="ti" @selected(old('client_document_type', $order->vehicle->client->document_type ?? '') === 'ti')>T.I.</option>
                                </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="phone" name="client_phone" placeholder="Digita número de teléfono"
                                value="{{ old('client_phone', $order->vehicle->client->phone ?? '') }}" required>
                            @error('client_phone')
                                <p class="text-danger small mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="text" id="address" name="client_address" placeholder="Digita dirección"
                                value="{{ old('client_address', $order->vehicle->client->address ?? '') }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="phone_driver" name="driver_phone"
                                ...
                                value="{{ old('driver_phone', $order->driver_phone ?? '') }}">
                            @error('driver_phone')
                                <p class="text-danger small mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="email" id="email" name="driver_email" placeholder="Digita correo electrónico"
                                value="{{ old('driver_email', $order->driver_email ?? '') }}">
                            @error('driver_email')
                                <p class="text-danger small mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="form-vehicle-data">
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="vehicle_observaciones" name="vehicle_observaciones" rows="2"
                                placeholder="Observaciones relevantes del vehículo">{{ old('vehicle_observaciones', $order->vehicle->observaciones ?? '') }}</textarea>
                        ...
                        <div class="flex flex-wrap items-center gap-4 text-base font-medium text-[#333]">
                            ...
                            <div class="flex items-center gap-2 text-sm text-[#333]">
                                <input type="radio" id="crane_yes" name="ingreso_en_grua" value="1"
                                    class="form-check-input" @checked(old('ingreso_en_grua', $order->ingreso_en_grua ? '1' : '0') === '1') required>
                                <label for="crane_yes" class="cursor-pointer">Sí</label>
                            </div>
                            <div class="flex items-center gap-2 text-sm text-[#333]">
                                <input type="radio" id="crane_no" name="ingreso_en_grua" value="0"
                                    class="form-check-input" @checked(old('ingreso_en_grua', $order->ingreso_en_grua ? '1' : '0') === '0')>
                ...
                <div class="data-client" id="witness-section">
                    ...
                    <div class="observation-container">
                        ...
                        @endphp
                        <div class="form-observations" data-witness-base="{{ asset('icons/witness') }}">
                            @foreach ($witnessGroups as $groupIndex => $group)
                                @php
                                    $containerClass =
                                        $groupIndex === 0 ? 'container-checkbox-one' : 'container-checkbox-two';
                                @endphp
                                <div class="{{ $containerClass }} flex flex-wrap items-center justify-center">
                                    @foreach ($group as $item)
                                        @php
                                            $rounding = $loop->first
                                                ? 'rounded-t-[1rem] md:rounded-tr-[0rem] md:rounded-l-[1rem]'
                                                : ($loop->last
                                                    ? 'rounded-b-[1rem] md:rounded-bl-[0rem] md:rounded-r-[1rem] '
                                                    : '');
                                            $label = 'Testigo ' . str_replace('witness-', '', $item);
                                        @endphp
                                        <button type="button"
                                            class="witness-indicator flex h-[90px] w-[90px] items-center justify-center bg-white border border-[#372C97] shadow-sm transition hover:shadow-md focus:outline-none {{ $rounding }}"
                                            data-witness-id="{{ $item }}">
                                            <img src="{{ asset("icons/witness/natural/{$item}.svg") }}"
                                                alt="{{ $label }}" class="h-16 w-16 object-contain"
                                                loading="lazy">
                                        </button>
                                    @endforeach
                                </div>
                        ...
                        <div class="flex flex-col items-center">
                            <div class="medidor">
                                <div class="fondo-medidor"></div>
                                <div class="aguja" id="aguja"></div>
                                <div class="centro"></div>
                            </div>

                            <div class="flex items-center gap-3 mt-14 w-full max-w-md z-30 pl-5">
                        ...
                        <div class="damage-section" data-damage-section="front">
                            ...
                            <div class="damage-canvas">
                                <svg width="427" height="305" viewBox="0 0 427 305" fill="none"
                                    ...
                                    <path
                                        d="M331.067 133.989C328.928 134.567 326.01 135.915 324.648 137.071C320.369 140.537 314.533 153.247 313.366 162.105L312.199 170.579L324.065 171.734C337.681 173.275 360.05 170.579 369.971 166.534C378.335 163.068 381.836 157.291 383.392 144.966C384.559 135.145 384.559 135.145 378.529 133.797C371.527 132.449 336.903 132.641 331.067 133.989ZM371.916 135.915L381.058 137.071L379.891 146.892C377.751 163.646 366.08 169.231 333.012 169.231H316.479V162.876C316.479 155.943 322.509 141.692 326.399 139.382C333.207 135.145 354.604 133.412 371.916 135.915Z"
                                        fill="#292D32" />
                                    <path
                                        d="M54.9226 134.031C50.6433 134.801 50.4488 135.379 51.8104 145.778C53.172 157.91 56.4788 162.724 65.6211 166.576C75.1524 170.62 97.7163 173.316 110.943 171.775L122.809 170.62L121.642 162.724C119.308 148.474 113.667 138.653 106.275 135.572C99.6615 132.875 66.3991 131.913 54.9226 134.031ZM106.664 138.268C112.111 141.156 116.584 149.629 118.141 160.606L119.308 169.272H102.579C68.7333 169.272 56.6733 163.495 55.1172 146.741L54.1446 136.727L60.5636 135.957C71.8456 134.416 102.19 135.764 106.664 138.268Z"
                                        fill="#292D32" />
                                    <path
                                        d="M58.6724 212.521C59.256 214.446 61.2012 221.187 62.7573 227.156C64.897 234.282 67.4257 239.096 69.9544 240.444C72.2886 241.599 82.598 242.947 93.1019 243.525C110.414 244.488 112.359 244.295 114.11 241.022C118.584 232.741 117.611 228.119 110.803 223.497C102.633 217.913 82.0144 211.365 68.0092 210.017C58.4779 208.862 57.6999 209.247 58.6724 212.521ZM78.1241 214.446C95.4361 218.298 112.165 225.616 113.721 229.66C114.499 231.393 113.915 235.052 112.748 237.748C110.608 242.562 110.414 242.562 93.4909 241.214C69.7599 239.289 68.9818 238.711 65.286 225.423C61.7847 212.906 61.3957 211.365 63.1463 211.365C63.7299 211.365 70.3434 212.713 78.1241 214.446Z"
                                        fill="#292D32" />
                                    <path
                                        d="M353.561 212.359C340.528 215.825 322.049 223.914 320.104 227.187C319.326 228.343 319.326 231.617 319.909 234.313C322.244 243.941 323.8 244.519 343.835 243.364C353.95 242.786 363.87 241.823 365.621 241.053C368.733 239.897 374.763 226.61 377.681 214.477C378.848 209.47 378.653 209.278 371.651 209.47C367.566 209.47 359.396 210.819 353.561 212.359ZM373.985 217.366C373.207 220.832 371.262 226.995 369.706 230.846C366.593 238.742 366.204 238.934 341.306 241.053C326.328 242.593 322.438 240.667 322.438 232.387C322.438 228.15 323.994 226.61 331.969 222.951C340.528 218.907 366.01 211.781 372.623 211.396C374.569 211.204 374.958 212.744 373.985 217.366Z"
                                        fill="#292D32" />
                                    <path
                                        d="M176.839 209.424V220.978H161.277C144.549 220.978 143.576 221.749 148.439 230.415C150.773 234.844 152.329 235.229 163.222 234.844C173.726 234.651 175.477 235.036 176.255 238.31C177.228 241.969 179.173 242.162 213.019 242.162C245.697 242.162 248.81 241.776 250.56 238.695C252.116 235.614 254.645 235.036 264.566 235.422C276.431 235.807 276.431 235.807 279.154 229.452C280.71 225.985 281.878 222.519 281.878 221.941C281.878 221.364 274.875 220.978 266.316 220.978H250.755V209.424V197.87H213.797H176.839V209.424ZM248.421 219.438L249.004 238.31H213.797H178.784V220.401C178.784 210.387 179.367 201.721 180.145 200.951C180.923 200.373 196.485 199.988 214.575 200.18L247.837 200.758L248.421 219.438ZM176.45 228.104C177.033 232.34 176.45 232.533 164.584 232.533C154.858 232.533 151.746 231.763 150.773 229.452C148.439 223.482 149.606 222.712 162.833 223.289C174.699 223.867 175.866 224.252 176.45 228.104ZM277.987 224.637C277.987 231.185 275.458 232.533 263.204 232.533C251.144 232.533 250.755 232.34 250.755 227.719C250.755 223.097 251.144 222.904 264.371 222.904C271.957 222.904 277.987 223.675 277.987 224.637Z"
                                        fill="#292D32" />
                                    <path
                            ...
                            <small class="damage-hint">Haz clic sobre el vehículo para marcar un punto. Haz clic en el
                                punto para
                                eliminarlo.</small>
                        ...
                        <div class="damage-section" data-damage-section="behind">
                            ...
                            <div class="damage-canvas">
                                <svg width="427" height="305" viewBox="0 0 427 305" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M130.097 1.75635C109.038 4.43762 94.297 10.2127 84.6099 19.494C75.5546 28.3628 62.2875 53.3193 55.5487 73.9444C50.9157 87.9695 48.8098 90.4445 47.1251 83.8445C45.4404 77.6569 39.7545 76.0069 20.5909 76.0069C10.9038 76.0069 7.11322 77.0382 3.95439 80.132C-1.52092 85.4945 -1.31033 88.1758 4.58616 96.8383C10.4826 105.501 17.2215 109.007 28.8039 109.007H37.8592L34.4898 115.401C27.3298 128.807 24.8027 189.652 30.278 215.227C32.3839 224.714 33.4368 241.627 33.4368 264.727C33.4368 289.477 34.2792 300.821 35.9639 302.471C39.5439 305.977 72.1852 305.771 75.344 302.265C78.2923 298.552 79.3452 292.159 79.5558 276.483C79.7664 264.933 80.1876 263.696 84.1887 263.696C86.7158 263.696 92.1911 259.983 96.8241 255.033L104.826 246.371L113.882 249.258C127.991 253.796 297.726 254.827 308.466 250.289C320.259 245.546 325.524 246.371 329.314 253.383C332.894 259.983 338.369 263.696 344.898 263.696C349.11 263.696 349.32 264.933 349.32 280.402C349.32 304.327 349.952 304.946 371.221 304.946C384.91 304.946 389.543 304.121 391.648 301.646C393.754 298.965 395.018 287.209 395.86 256.683C396.703 233.789
                                        d="M296.811 85.8898C285.924 87.3727 283.37 89.3006 283.37 96.1222C283.37 101.164 285.251 101.906 297.617 101.906C306.354 101.906 307.698 101.461 309.445 98.4949C313.074 92.4148 313.478 87.521 310.521 85.7415C308.908 84.8517 307.564 84.2585 307.295 84.2585C307.026 84.4068 302.322 85.1483 296.811 85.8898ZM310.252 90.1904C310.252 97.7535 308.236 98.9398 296.139 98.9398C286.058 98.9398 284.714 98.6432 284.714 96.2705C284.714 89.7455 288.343 87.9659 304.607 87.3727C309.445 87.0761 310.252 87.521 310.252 90.1904Z"
                                        fill="#292D32" />
                                    <path
                                        d="M419.097 76.7726C410.226 78.1072 407.134 80.9249 407.806 87.1533C408.21 90.7124 408.478 90.8607 420.71 91.3056L433.21 91.7505L435.226 86.1152C438.99 76.476 435.495 74.3998 419.097 76.7726ZM435.092 78.2555C435.495 78.997 434.957 81.9629 433.882 84.9289L431.866 90.2675L420.71 89.8226C411.973 89.526 409.554 88.9329 409.15 87.1533C408.344 82.5561 412.107 79.8868 420.979 78.5521C433.21 76.6243 434.151 76.6243 435.092 78.2555Z"
                                        fill="#292D32" />
                                    <path
                                        d="M487.975 75.3251C487.302 76.3632 486.227 80.0706 485.69 83.4814C485.017 88.5235 485.286 90.1548 487.571 92.6758C489.856 95.1969 492.007 95.79 500.206 95.79C506.254 95.79 510.959 95.0486 512.437 93.8622C515.529 91.3411 517.007 79.0326 514.588 75.77C512.437 72.8041 490.125 72.5075 487.975 75.3251ZM513.109 77.1047C514.319 78.5877 514.453 81.257 513.647 85.7059L512.437 92.0826H500.475H488.378L487.975 85.7059C487.706 82.1468 488.243 78.2911 489.184 77.1047C491.335 74.1388 510.959 74.1388 513.109 77.1047Z"
                                        fill="#292D32" />
                                    <path
                                        d="M449.09 107.382C445.057 108.865 441.159 110.941 440.353 111.831C439.412 113.017 439.546 113.314 441.025 112.721C455.004 106.937 461.993 105.454 471.805 106.196C477.585 106.641 485.515 108.42 489.95 110.348C500.838 114.945 512.262 126.957 517.773 139.266C520.193 144.456 522.343 148.46 522.747 148.164C523.956 146.829 516.429 131.703 511.322 125.178C505.139 117.021 494.789 109.755 485.38 106.641C475.434 103.378 458.498 103.675 449.09 107.382Z"
                                        fill="#292D32" />
                                    <path
                                        d="M563.082 164.007C561.469 164.303 559.453 165.49 558.646 166.676C556.63 169.345 562.141 172.608 570.878 174.091C575.717 174.832 577.464 174.536 579.211 172.608C582.034 169.494 582.168 165.045 579.346 163.858C577.061 162.968 567.921 162.968 563.082 164.007ZM579.615 166.379C580.018 166.676 579.883 168.159 579.346 169.494C578.27 172.459 569.265 172.608 563.351 169.79L559.319 167.862L563.351 166.676C565.636 166.083 568.055 165.49 568.727 165.045C570.206 164.451 578.808 165.49 579.615 166.379Z"
                                        fill="#292D32" />
                                </svg>
                            ...
                            <div class="damage-canvas">
                                <svg width="597" height="211" viewBox="0 0 597 211" fill="none"
                                    ...
                                    <path
                                        ...
                                        fill="black" />
                                    <path
                                        d="M17.2007 165.111C13.9645 167.895 15.4478 173.316 19.7627 174.488C23.808 175.514 35.9439 172.584 37.9665 169.946C40.124 167.016 36.6181 164.965 27.7184 163.939C21.9202 163.353 19.0885 163.646 17.2007 165.111ZM34.4606 167.309C38.371 168.481 35.1348 170.679 27.3139 171.998C21.6505 173.023 19.8976 172.73 18.1446 170.972C16.2568 168.921 16.3917 168.481 18.5491 166.723C20.9763 164.818 27.1791 164.965 34.4606 167.309Z"
                                        fill="black" />
                                    <path
                                        d="M114.012 106.971C99.7185 110.634 85.2903 123.381 78.1436 138.913C75.8513 143.748 74.368 147.851 75.0422 147.851C75.5816 147.851 77.7391 144.188 79.7617 139.792C92.0324 112.539 122.642 99.9379 149.61 111.074C157.971 114.59 157.971 114.59 156.892 112.685C156.487 111.953 152.172 109.901 147.318 108.29C137.879 105.213 123.316 104.627 114.012 106.971Z"
                                        fill="black" />
                                    <path
                                        d="M118.822 119.131C97.9215 125.578 85.516 144.333 86.8644 167.63C88.3477 192.392 106.282 211 128.666 211C146.33 211 160.084 201.916 167.77 185.066C176.535 165.725 172.624 144.186 157.927 129.827C147.544 119.717 131.497 115.321 118.822 119.131ZM144.038 122.647C153.072 126.017 164.804 138.765 167.905 148.582C169.119 152.538 170.197 160.303 170.197 165.578C170.197 203.234 128.396 223.308 102.641 198.106C92.5278 188.289 88.7522 179.351 88.7522 164.846C88.6174 145.798 95.7641 132.904 111.136 124.552C117.878 121.036 121.114 120.156 128.396 120.156C133.25 120.156 140.397 121.329 144.038 122.647Z"
                                        fill="black" />
                                    <path
                                        d="M465.879 119.131C444.979 125.578 432.573 144.333 433.922 167.63C435.405 192.392 453.339 211 475.723 211C493.387 211 507.141 201.916 514.827 185.066C523.592 165.725 519.682 144.186 504.984 129.827C494.601 119.717 478.555 115.321 465.879 119.131ZM491.095 122.647C500.129 126.017 511.861 138.765 514.962 148.582C516.176 152.538 517.254 160.303 517.254 165.578C517.254 203.234 475.453 223.308 449.698 198.106C439.585 188.289 435.809 179.351 435.809 164.846C435.675 145.798 442.821 132.904 458.193 124.552C464.935 121.036 468.172 120.156 475.453 120.156C480.307 120.156 487.454 121.329 491.095 122.647Z"
                                        fill="black" />
                                    <path
                                        d="M118.583 136.694C94.3114 150.907 100.514 188.27 127.483 191.054C136.382 192.08 144.608 187.977 150.811 179.479C159.44 167.757 156.339 147.683 144.608 139.185C137.731 134.35 124.786 133.031 118.583 136.694ZM139.079 138.16C145.821 141.236 148.788 144.313 151.754 151.346C159.575 169.808 147.844 189.149 129.101 189.149C108.065 189.149 97.5476 162.629 111.571 145.193C118.853 136.108 128.966 133.617 139.079 138.16Z"
                                        fill="black" />
                                    <path
                                        d="M465.641 138.224C441.369 152.437 447.572 189.8 474.541 192.584C483.44 193.609 491.666 189.507 497.868 181.008C506.498 169.287 503.397 149.213 491.666 140.715C484.789 135.88 471.844 134.561 465.641 138.224ZM486.137 139.689C492.879 142.766 495.846 145.843 498.812 152.876C506.633 171.338 494.902 190.679 476.159 190.679C455.123 190.679 444.605 164.158 458.629 146.722C465.911 137.638 476.024 135.147 486.137 139.689Z"
                                        fill="black" />
                                    <path
                                        d="M86.4512 74.9629C84.5634 78.626 85.9119 88.736 88.7436 91.6664C90.092 93.2782 93.7328 93.8643 101.149 93.8643C114.229 93.8643 116.926 91.6664 115.577 81.8495C114.229 72.4721 113.285 71.886 99.8007 71.886C89.1481 71.886 87.7997 72.179 86.4512 74.9629ZM111.667 75.1095C114.229 78.626 114.768 87.8569 112.611 90.2012C110.049 92.9851 93.4631 93.2782 90.092 90.4943C87.7997 88.5895 86.5861 78.1864 88.339 75.1095C89.6875 72.6186 109.914 72.7651 111.667 75.1095Z"
                                        fill="black" />
                                    <path
                                        d="M156.595 73.4378C155.381 75.0496 155.381 76.8078 156.73 81.2035L158.617 86.9178L171.427 87.3574C185.721 87.797 186.934 87.0644 183.968 79.2987C182.08 74.4635 181.945 74.4635 164.955 72.2657C159.966 71.5331 157.808 71.8261 156.595 73.4378ZM181.675 78.5661C182.484 79.7383 183.294 82.0826 183.294 83.5478C183.294 86.0387 182.215 86.3318 171.562 85.8922C160.101 85.4526 159.831 85.3061 158.617 81.35C156.46 74.0239 157.539 73.4378 169.405 74.7565C176.417 75.6357 180.462 76.9544 181.675 78.5661Z"
                                        fill="black" />
                                    <path
                                        d="M292.09 81.0853C290.876 82.697 290.876 84.4553 292.225 88.851L294.112 94.5653L306.923 95.0049C321.216 95.4444 322.429 94.7118 319.463 86.9462C317.575 82.111 317.44 82.111 300.45 79.9131C295.461 79.1805 293.303 79.4736 292.09 81.0853ZM317.171 86.2136C317.98 87.3857 318.789 89.7301 318.789 91.1953C318.789 93.6862 317.71 93.9792 307.057 93.5397C295.596 93.1001 295.326 92.9536 294.112 88.9975C291.955 81.6714 293.034 81.0853 304.9 82.404C311.912 83.2831 315.957 84.6018 317.171 86.2136Z"
                                        fill="black" />
                                    <path
                                        d="M555.729 154.01C556.133 158.113 557.617 163.095 559.37 165.732C562.201 169.981 562.876 170.274 571.371 170.274C583.641 170.274 587.012 168.223 587.552 160.75C587.687 157.527 587.282 154.01 586.743 152.985C584.99 150.201 572.18 146.831 563.01 146.831H555.055L555.729 154.01ZM580.135 151.227C584.99 152.838 585.125 153.131 585.125 159.432C585.125 167.197 584.181 167.784 571.775 167.93L563.01 168.077L559.909 162.216C558.291 158.992 556.808 154.45 556.808 152.106V147.857L565.977 148.736C570.966 149.322 577.304 150.347 580.135 151.227Z"
                                        fill="black" />
                                </svg>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (witnessContainer && witnessStatesInput) {
                // Cargar estados iniciales de testigos
                try {
                    const savedStates = JSON.parse(witnessStatesInput.value || '{}');
                    witnessContainer.querySelectorAll('.witness-indicator').forEach((indicator) => {
                        const id = indicator.dataset.witnessId;
                        if (id && savedStates[id]) {
                            const state = savedStates[id];
                            indicator.dataset.witnessState = state;
                            const img = indicator.querySelector('img');
                            if (img) {
                                const basePath = witnessContainer.dataset.witnessBase || '/icons/witness';
                                img.src = `${basePath}/${state}/${id}.svg`;
                            }
                        }
                        indicator.addEventListener('click', () => syncWitnessStates());
                    });
                } catch (error) {
                    console.error('Error loading witness states:', error);
                    witnessContainer.querySelectorAll('.witness-indicator').forEach((indicator) => {
                        indicator.addEventListener('click', () => syncWitnessStates());
                    });
                }
